...
**Integrity:** use DB checks for lifecycle enums, uint256 bounds, and royalty range; unique live-listing constraint is a partial index. Enforce that a transaction’s listing belongs to its artwork and chain, and listing seller wallet matches the authenticated owner at listing creation. Addresses/URIs/hash/calldata use validated text; block/log numbers are nonnegative integers. Server controls identity, status, timestamps, ownership, and settlement fields; reject attempts to set them through metadata/profile writes. Draft `owner_wallet` is provisional until mint confirmation.
...
2. **Creation/metadata:** valid upload → draft → pinned JSON with the exact field mapping; reject oversized/spoofed files, missing title, invalid attributes/URI, out-of-range royalties, and another user’s upload receipt. Freeze pending/minted metadata; failed mint permits editing.
3. **Mint/list:** unconfirmed receipts never publish ownership/listings; verified confirmation does. Duplicate preparation/submission/event replay creates no duplicate token or listing. Cross-artwork listing references and forged server-managed fields fail. Price is exact; zero/negative/overflow prices fail.
